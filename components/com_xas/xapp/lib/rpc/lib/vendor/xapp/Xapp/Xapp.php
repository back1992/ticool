...
abstract class Xapp
{
    ...
    public static function run(Array $conf = null)
    {
        ...
        if(xapp_conf(XAPP_CONF_AUTOLOAD) !== null && xapp_conf(XAPP_CONF_AUTOLOAD) !== false)
        {
            if(!(xapp_conf(XAPP_CONF_AUTOLOAD) instanceof Xapp_Autoloader))
            {
                ...
                {
                    Xapp_Autoloader::run(xapp_conf(XAPP_CONF_AUTOLOAD));
                }
            }
        }
    ...
    public static function assertOptions($options = null, $object = null)
    {
        try
        {
            ...
            foreach($rule as $k => $v)
            {
                ...
                {
                    throw new Xapp_Error("unable to initialize options since option: $k must be set", 1010701);
                }
            }
        }
        catch(ReflectionException $e)
        {
            throw new Xapp_Error("reflection error: " . $e->getMessage(), 1010702);
        }
    }


    /**
     * xapp option helper init option
     * receive options and delegate first to xapp::assertOptions and then to xapp::setOptions
     *
    ...
    /**
     * xapp option helper set options
     * sets multiple options using xapps setOption helper
     *
     * @see xapp::setOption
     * @error 10109
    ...
    /**
     * xapp option helper set option
     * sets single option to options array of passed object checking if options
     * dictionary $optionsDict is set to validate passed option value for data
     * type matching and will throw error if data type does not match
     *
     * @error 10110
    ...
    public static function setOption($key, $value = null, $object = null, $reset = false)
    {
        ...
        if(!empty($dict))
        {
            if(array_key_exists($key, $dict))
            {
                    ...
                    try
                    {
                        if(defined('XAPP_TYPE_' . strtoupper($v)))
                        {
                            if(strtoupper($v) !== XAPP_TYPE_MIXED)
                            {
                                if(!xapp_is($v, $value))
                                {
                                    throw new Xapp_Error("unable to set option: $key due to missing or invalid dataype", 1011001, XAPP_ERROR_IGNORE);
                                }else{
                                    $ok++;
                                }
                            }
                            ...
                            {
                                throw new Xapp_Error("unable to set option: $key since value must be instance of: $v", 1011002, XAPP_ERROR_IGNORE);
                            }else{
                                $ok++;
                            }
                        }
                ...
                {
                    $err = array_shift($err);
                    error_log("xc option : failed: " . $key );
                    throw new Xapp_Error($err->getMessage(), $err->getCode(), XAPP_ERROR_ERROR);
                }
            }else{
                throw new Xapp_Error("the option: $key is not defined in the options dictionary", 1011003);
            }
        }
        ...
        if(!(bool)$reset && isset($options[$key]) && is_array($options[$key]) && is_array($value))
        {
            ...
            {
                $options[$key] = array_unique(array_merge($options[$key], $value));
            }else{
    ...
    /**
     * xapp option helper has option
     * checks if option key does exists in options array of passed object using
     * either strict checking or not where strict = true $key value must have a value
     * other then null|false|empty.
     *
     * @error 10112
     * @param null|string $key expects the option key name
     * @param null|string|object $object expects the object to assert option values for as string or object reference
    ...
    public static function hasOption($key = null, $object = null, $strict = false)
    {
        if($key !== null)
        {
            ...
            if(is_object($object) && array_key_exists('options', get_class_vars(get_class($object))))
            {
                return ((bool)$strict) ? (array_key_exists($key, $object->options) && xapp_is('value', $object->options[$key])) : (array_key_exists($key, $object->options));
            }else{
                $options = Xapp_Reflection::propertyFactory($object, 'options', 'NIL', true);
    ...
    /**
     * xapp option helper get options
     * returns options array from object
     *
     * @error 10113
     * @param null|string|object $object expects the object to assert option values for as string or object reference
     * @return null|array options array
     */
    public static function getOptions($object = null)
    {
        if($object === null)
        {
            $object = self::optionCallee();
        }
        if(is_object($object) && in_array('option', get_class_vars(get_class($object))))
        {
    ...
    /**
     * xapp option helper get option
     * get single option value from option array of passed object returning default
     * value if option key is not found in option array
     *
     * @error 10114
     * @param string $key expects the option key name
     * @param null|string|object $object expects the object to assert option values for as string or object reference
    ...
    public static function getOption($key, $object = null, $default = null)
    {
        ...
        if(is_object($object) && in_array('options', get_class_vars(get_class($object))))
        {
    ...
    /**
     * xapp option helper reset options
     * reset objects option array by reseting it to empty array
     *
     * @error 10115
     * @param null|string|object $object expects the object to assert option values for as string or object reference
     * @return void
    ...
    /**
     * xapp option helper reset option
     * reset an option key by value making sure that value can not be appended for array values. this function
     * uses the setOption function and the reset flag set to true
     *
     * @error 10129
     * @param string $key expects the option key name
     * @param null|mixed $value expects the option value
    ...
    public function __call($method, Array $args = null)
    {
        ...
        if(substr(strtolower($method), 0, 3) === 'get' || substr(strtolower($method), 0, 3) === 'set')
        {
            ...
            if(Xapp_Reflection::hasProperty($this, '_' . $property))
            {
                ...
                throw new Xapp_Error("property: $property does not exist an can not be called via overloading", 1011502);
            }
        }else{
            throw new Xapp_Error("method: $method does not exist and can not be called via overloading", 1011501);
        }
    }
    ...
    protected static function getClassName($mixed, $lower = false)
    {
        if(is_string($mixed))
        {
            ...
            throw new Xapp_Error("unable to determine class name from passed value: $mixed", 1011901);
        }
    }
