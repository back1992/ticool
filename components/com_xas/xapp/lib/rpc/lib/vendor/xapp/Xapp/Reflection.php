        ...
        try
        {
            ...
            if(class_exists($class, true))
            {
                ...
                if(!$cls->isAbstract())
                {
                    ...
                    throw new Xapp_Error("class: $_class can not be instantiated since class is abstract", 1060203);
                }
            }else{
                throw new Xapp_Error("class: $_class does not exist", 1060201);
            }
        }
        catch(ReflectionException $e)
        {
        ...
        try
        {
            ...
            if(method_exists($class, $method))
            {
                ...
                if(!$mtd->isProtected())
                {
                    ...
                    throw new Xapp_Error("method: $method of class: $_class can not be invoked since method is protected", 1060303);
                }
            }else{
                throw new Xapp_Error("class: $_class or method: $method does not exist", 1060301);
            }
        }
        catch(ReflectionException $e)
        {
            ...
            if($obj->hasProperty($property))
            {
                ...
                if($prop->isStatic())
                {
                    ...
                    if(array_key_exists($property, $props))
                    {
                        ...
                        throw new Xapp_Error("static property: $property does not exist in class: $_class", 1060404);
                    }
                }else{
                    if(is_object($class))
                    {
                        ...
                        throw new Xapp_Error("unable to set/get non static property for class: $_class passed as string", 1060403);
                    }
                }
            }else{
                throw new Xapp_Error("class: $_class or property: $property does not exist", 1060402);
            }
        }
        catch(ReflectionException $e)
        {
        ...
        try
        {
            ...
            if($const !== null)
            {
                ...
                if($obj->hasConstant(strtolower($const)) || $obj->hasConstant(strtoupper($const)) )
                {
                    ...
                    throw new Xapp_Error("const: $const does not exist in class: $_class", 1060502);
                }
            }else{
