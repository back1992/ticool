...
class Xapp_Util_Std_Query implements Iterator
{
    ...
    {
        xapp_init_options($options, get_class());
        if(is_array($object) || is_array($object))
        {
            $this->_object =& $object;
        }else{
            throw new Xapp_Util_Std_Exception("passed object in constructor is not an object or array", 1670101);
        }
    }
    ...
    public function first()
    {
        if($this->_result !== null)
        {
            ...
            throw new Xapp_Util_Std_Exception("method: first() used in wrong context", 1670601);
        }
    }
    ...
    public function last()
    {
        if($this->_result !== null)
        {
            ...
            throw new Xapp_Util_Std_Exception("method: last() used in wrong context", 1670701);
        }
    }
    ...
    public function at($index = 0)
    {
        if($this->_result !== null)
        {
            if(is_array($this->_result) && array_key_exists((int)$index, $this->_result))
            {
                ...
                if(xapp_get_option(self::THROW_EXCEPTION, get_class()))
                {
                    throw new Xapp_Result_Exception("no object found at index", 1670801);
                }else{
                    return false;
                }
            }
        }else{
            throw new Xapp_Util_Std_Exception("method: at() used in wrong context", 1670801);
        }
    }
        ...
        if(empty($path))
        {
            ...
            foreach(explode('/', $path) as $p)
            {
                ...
                if(is_numeric($p) && (intval($p) == floatval($p)))
                {
                    ...
                    {
                        if($default === false && xapp_get_option(self::THROW_EXCEPTION, get_class()))
                        {
                            throw new Xapp_Result_Exception("no result found for path", 1671401);
                        }else{
                            return $default;
                        }
                    }else{
