...
class Xapp_Event
{
    ...
    public static function listen($event, $callback, $reset = false)
   	{
        ...
        {
            throw new Xapp_Error("invalid callback function: $var supplied for event: $event", 1040201);
        }
        if(is_array($callback) && !is_callable($callback, true, $var))
        {
            throw new Xapp_Error("invalid callback class/method: $var supplied for event: $event", 1040202);
        }
    ...
    public static function listeners($event = null, $default = array())
    {
        if($event !== null)
        {
            return (isset(self::$_events[$event])) ? self::$_events[$event] : $default;
        }else{
            return (sizeof(self::$_events) > 0) ? self::$_events : $default;
        }
    }
            ...
            if(isset(self::$_events[$event]))
            {
                if($index !== null)
                {
                    if(is_int($index) && array_key_exists($index, self::$_events[$event])){
                        ...
                        throw new Xapp_Error("unable to reset listener for event name and index: $index" , 1040501);
                    }
                }else{
                    self::$_events[$event] = array();
                }
