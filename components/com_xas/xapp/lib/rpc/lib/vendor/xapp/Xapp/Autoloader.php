...
class Xapp_Autoloader implements Xapp_Singleton_Interface
{
    ...
    protected function __construct($dirs = null, $options = null)
    {
        ...
        if(strnatcmp(phpversion(), '5.3.0') < 0 || !function_exists('spl_autoload_register'))
        {
            if(!function_exists('__autoload'))
            {
                ...
                throw new Xapp_Error("xapp autoloader can not overwrite autoloading since __autoload is already set");
            }
        }else{
    ...
    protected function init($dirs = null)
    {
        ...
        {
            $dirs = array_unique(array_merge((array)$dirs, xapp_get_option(self::DIRECTORIES, $this)));
        }
            ...
            foreach($dirs as $key => $dir)
            {
                ...
                if(is_dir($dir[0]))
                {
                    ...
                    if($tmp !== null)
                    {
                        ...
                        throw new Xapp_Error(xapp_sprintf("relative dir: %s is not a valid dir", array($dir[0])), 1070901);
                    }
                }
            }
        }
    ...
    public function preload($dir = null)
    {
        ...
        foreach((array)$dir as $k => $v)
        {
            ...
            if(is_dir($v[0]))
            {
                ...
                {
                    throw new Xapp_Error(xapp_sprintf("iterator exception: %d - %s", array($e->getCode(), $e->getMessage())), 0170501);
                }
            }else{
                throw new Xapp_Error(xapp_sprintf("unable to preload dir: %s since dir is not valid", $v[0]), 0170501);
            }
        }
        return $this->_classes;
    }
