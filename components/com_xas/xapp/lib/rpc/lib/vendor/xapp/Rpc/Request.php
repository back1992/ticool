...
abstract class Xapp_Rpc_Request
{
    ...
    public static function factory($driver, $options = null)
    {
        ...
        if(class_exists($class, true))
        {
           return new $class($options);
            ...
            throw new Xapp_Rpc_Request_Exception("rpc request class: $class not found", 1440201);
        }
    }
    ...
    protected function init()
    {
        ...
        if($this->_url === null)
        {
            ...
            {
                $this->_charset = strtolower(trim($m[1]));
                if(isset($m[2]))
                {
                    $this->_contentType = strtolower(trim($m[2]));
                }
            }
    ...
    public function getFrom($name, $from = self::PARAMS, $default = null)
    {
        ...
        switch($from)
        {
            ...
            case self::SERVER:
                ...
                break;
            default:
                throw new Xapp_Rpc_Request_Exception("request variable scope: $from does not exist", 1441001);
        }
        return $return;
    }
    ...
    public function set($name, $value = null, $to = self::PARAMS)
    {
        ...
        switch($to)
        {
            ...
            case self::COOKIE:
                ...
                break;
            default:
                throw new Xapp_Rpc_Request_Exception("request variable scope: $to does not exist", 1441401);
        }
        return $this;
    }
    ...
    public static function url($url = null, $component = null)
    {
        ...
        if($component !== null)
        {
            if(($u = parse_url($url, (int)$component)) !== false)
            {
                ...
                throw new Xapp_Rpc_Request_Exception("url: $url is not a valid url", 1442001);
            }
        }else{
            return $url;
        }
    ...
    public function __get($name)
    {
        if($this->has($name))
        {
            ...
            throw new Xapp_Rpc_Request_Exception("overloading properties only allowed for registered global variables", 1442401);
        }
    }
    ...
    public function __call($name, Array $arguments = array())
    {
        if(empty($arguments))
        {
            if($this->has($name))
            {
                throw new Xapp_Rpc_Request_Exception("overloading methods only allowed for registered global variables", 1442701);
            }else{
                return $this->get($name);
            }
        }else{
