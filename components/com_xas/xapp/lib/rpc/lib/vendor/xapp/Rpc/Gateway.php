...
class Xapp_Rpc_Gateway implements Xapp_Singleton_Interface
{
    ...
    public function conf($conf)
    {
        ...
        if(is_file($conf))
        {
            ...
            {
                throw new Xapp_Rpc_Gateway_Exception("unable to open rpc conf file", 1401401);
            }
        }
        $conf = trim($conf, "\r\n");
        $conf = explode("\n", $conf);
    ...
    protected function validate($option, $value = null)
    {
        ...
        switch($option)
        {
            ...
            case self::DISABLE_SERVICE:
                if($this->server()->service() !== null && !preg_match('/(\b'.implode('\b$|\b', trim($value, '()\\b$ ')).'\b$)/i', $this->server()->service()))
                {
                    Xapp_Rpc_Fault::t("requested service is disabled", array(1401505, -32005));
                }
                break;
            ...
            case self::SIGNED_REQUEST:
                if($value)
                {
                    if(xapp_is_option(self::SIGNED_REQUEST_EXCLUDES, $this) && (in_array($this->server()->service(), xapp_get_option(self::SIGNED_REQUEST_EXCLUDES, $this)) || preg_match('/^'.implode('|', xapp_get_option(self::SIGNED_REQUEST_EXCLUDES, $this)).'$/i', $this->server()->service())))
                    {
                        //do nothing since service is excluded from signed services exclude array
                    }else{
                        ...
                        switch($method)
                        {
                            ...
                            case 'user':
                                ...
                                break;
                            default:
                                throw new Xapp_Rpc_Gateway_Exception("unsupported signed request user identification method", 1401514);
                        }
    ...
    final public function run()
    {
        ...
        $this->server()->setup();
        if($this->server()->service() !== null)
        {
            if(self::VALIDATE===true){

                //error_log('validate rpc gateway');
