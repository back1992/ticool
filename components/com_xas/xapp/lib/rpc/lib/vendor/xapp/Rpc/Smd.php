...
abstract class Xapp_Rpc_Smd implements Xapp_Singleton_Interface
{
    ...
    /**
     * define whether to use method as service where function/method and or class is passed in url
     ...
     */
    const METHOD_AS_SERVICE          = 'RPC_SMD_METHOD_AS_SERVICE';
    ...
    public static function factory($driver, $options = null)
    {
        ...
        if(class_exists($class, true))
        {
            ...
            throw new Xapp_Rpc_Smd_Exception("rpc smd class: $class not found", 1410101);
        }
    }
     ...
     * @param array $ignore expects methods to ignore
     * @return array
     */
    final public function map($mixed = null, Array $ignore = array())
    {
        if($mixed !== null)
        {
            if(!is_object($mixed))
            {
                if(strpos($mixed, DS) !== false && is_dir($mixed))
                {
     ...
     * @return Xapp_Rpc_Smd
     */
    public function set($mixed = null, Array $ignore = array())
    {
    ...
    protected function mapFunction($function)
    {
        try
        {
            ...
            {
                throw new Xapp_Rpc_Smd_Exception("function: $function can not be mapped since it is internal", 1410502);
            }
            $obj->transport = null;
            $obj->target = null;
            ...
            foreach($function->getParameters() as $p)
            {
                ...
                if(isset($method->params) && isset($method->params['params'][$i]))
                {
                    $params->type = $method->params['params'][$i];
                }
        ...
        {
            throw new Xapp_Rpc_Smd_Exception(xapp_sprintf("unable to map function: $function due to reflection error: %d - %s", array($e->getCode(), $e->getMessage())), 1410501);
        }
    }
     ...
     * @param array $ignore expects optional array of methods to ignore
     * @return array
     * @throws Xapp_Rpc_Smd_Exception
     */
    protected function mapClass($class, Array $ignore = array())
    {
        $map = array();
        ...
        try
        {
            ...
            $_methods = array_merge((array)xapp_get_option(self::IGNORE_METHODS, $this), $ignore);
        ...
        {
            throw new Xapp_Rpc_Smd_Exception(xapp_sprintf("unable to map class: $_class due to reflection error: %d - %s", array($e->getCode(), $e->getMessage())), 1410601);
        }
    }
    ...
    protected function getClasses($dir, &$tmp = array())
    {
        ...
        if(($d = @opendir(rtrim($dir, DS))) !== false)
        {
            ...
            throw new Xapp_Rpc_Smd_Exception("class dir: $dir is not found", 1411001);
        }
        return $tmp;
    }
    ...
    protected function parseClass($class, &$tmp = array())
    {
        if(($fp = fopen($class, 'r')) !== false)
        {
            ...
            throw new Xapp_Rpc_Smd_Exception("unable to open: $class", 1411101);
        }
    }
    ...
    protected function getTarget($service = null, $relative = null)
    {
        ...
        if($service !== null)
        {
            ...
            {
                $class = substr($service, 0, strpos($service, '.'));
                $function = substr($service, strpos($service, '.') + 1);
            }else{
                $class = null;
                $function = $service;
            }
        ...
        if($service !== null)
        {
            if(xapp_is_option(self::REWRITE_URL, $this))
            {
                ...
                {
                    $_url = rtrim($path, '/') . '/{$class}/{$function}';
                }
                $_url = trim($_url, '/ ');
                if(($_url = parse_url($_url)) !== false)
                {
                    ...
                    throw new Xapp_Rpc_Smd_Exception("smd rewrite rule; $_url is not a valid url or url path value", 1411301);
                }
            }else{
