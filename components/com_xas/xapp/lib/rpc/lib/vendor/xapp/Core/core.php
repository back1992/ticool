...
// *****************************************************************
// xapp core functions. xapp core.php must always be included
// regardless of what module to use. core.php sets global config
// path, ini, env values and contains all important constants
// core.php is the very heart of xapp containing also global
// interfaces, facades and most important all pass through functions
// that can be called from any class and redirects it logic to xapp
// base classes if loaded. xapp core.php is supposed to run without
// the Xapp base class under xapp/Xapp/Xapp.php
// *****************************************************************
...
// *****************************************************************
// XAPP_ERROR
// error level constants to use system wide in all Xapp_Error and
// sub class exception calls
// *****************************************************************
...
// *****************************************************************
// XAPP_TYPE
// data type constants to be used for xapps options handling defining
// the type of option data type to be allowed as option value
// *****************************************************************
...
// *****************************************************************
// XAPP_PATH
// all path values are set and retrieved by xapp_path function. if
// called without any parameters the first time xapp_path will init
// all paths and determine their default values.
// *****************************************************************
...
// *****************************************************************
// XAPP_CONF
// all the following values conf options have only affect if they are
// used together with xapp base class Xapp.php as application base class
// and calling xapp::run to start app. the conf values can be initialized
// inside Xapp.php by passing conf array through xapp::run or by calling
// xapp_conf with the same conf array
// *****************************************************************

/**
 * conf value charset, if set, expects a valid charset value to be set.
 * e.g. default value is "utf-8" but can by any valid I18N charset. the
 * value will be set in xapp::run if xapp is used and will set the internal
 * app encoding. if the value is set to null|false nothing will be set.
 *
...
/**
 * conf value language, if set, expects a valid language string used in setting
 * php environment var "language" which is usually the same used for setlocale
 * e.g. "en.UTF8". if value is null|false nothing will be set.
 *
...
/**
 * conf value locale, if set, expects a valid locate name which is used
 * got setlocale phps native function, e.g. "en.UTF8". the value can be set
 * as single string or array containing multiple values like:
 *
 * <code>
...
/**
 * conf value timezone, if set, expects the default timezone value as used in
 * php natives function date_default_timezone_set, e.g. UTC. if value is null|false
 * nothing will be set.
 *
 ...
 * 1)   if set to true = initialize Xapp_Autoloader inside xapp to use internal autoloader
 *      with default options
 * 2)   if set with array = initialize Xapp_Autoloader with specific options - see
 *      Xapp_Autoloader
 ...
 * 1)   if set to true = sets error handler via php natives set_error_handler function
 *      to Xapp_Error error handling and be doing so let that class handle all the
 *      errors with default settings
 * 2)   if set to object or callable = redirects all error handling to that callable
 *      or object
 * 3)   if set to null|false = nothing will be set
 *
...
/**
 * conf value handle_exception, if set, does the same as XAPP_CONF_HANDLE_ERROR but just
 * taking care of handling exceptions instead of errors.
 *
 ...
 * 1)   if set to true = registers internal xapp base class shutdown function to handle
 *      debug and error message as well as flush all contents
 * 2)   if set to object or callable = sets shutdown according to value passed
 * 3)   if set to null|false = nothing will be set
 * NOTE: when using xapp base module shutdown handler should not be changed and constant
 * passed with true. This will take that error handling will be done right
 *
 ...
 * NOTE: when using xapp base module buffer handling should be left with value true since
 * xapp will take care of settings headers and output compression.
 *
...
/**
 * conf value handle_error_codes, if set, can pass php native error code bitmasked constants
 * for internal error handling and displaying. the level of error logging, if using xapp
 * to take of error handling, is set like you would with php natives error_reporting
 * function. so:
 * 1)   if set to null|false = xapp will use the php default value or take the values set
 *      in error_reporting before if set.
 * 2)   if set to int = e.g. E_ALL will use the bitmasked value to set error_reporing inside
 *      xapp and therefore pass these value also to the error handling capacities of xapp
 ...
 * 1)   if set to true = will try to initialize Xapp_Log_Error to be used as default error
 *      logger with default options
 * 2)   if set to Xapp_Log_Error instance = will take instance to be used as error logger
 *      with all logger options set
 * 3)   if set to object = will look for public method log($message) in passed instance because
 *      only then 3rd party logger will be accepted to work with xapp base class
 * 4)   if set as string representing a log file absolute file or dir path will log all errors
 *      using xapp error logger class or phps native error_log function to passed log file or directory
...
/**
 * conf value memory_limit, if set, will set the php native ini value memory_limit by expecting
 * as value other then null|false
 *
...
/**
 * conf value execution_time, if set, will set the php native ini value execution time through phps native
 * function set_time_limit expecting a value other then null|false
 * NOTE: if xapp is run under cli execution time is overwritten by 0 = endless
 *
 ...
 * 1)   all argument missing php warning will throw errors making sure that all missing argument
 *      warning, which are errors!, are forced to correct!
 *
 ...
 * 4)   if set to 4 = will be routed directly to php console if Xapp_Console is used or driver has been
 *      specified for XAPP_CONF_CONSOLE
 * 5)   if set to "console" see 4)
 * 6)   if set to null|false = ignore debug messages and discard them
 * NOTE: since all debug messages are first send to xapp_debug function all debug methods can
 * be intercepted by overwriting xapp_debug
 *
...
/**
 * conf value profiler mode, if set and using xapp base class Xapp.php will auto profile xapp native
 * functionality like xapp start, shutdown and sql queries and also any custom profiling.
 ...
 * NOTE: since all profiling messages are first send to xapp_profile function all profiling data can be
 * intercepted by overwriting xapp_profile
 *
...
/**
 * conf value system_email, if set, is used in conjunction with xapp internal error logging
 * to dispatch severe errors to the passed e-mail address expecting the mail system to work
 * if set to null|false remains unused
 *
...
/**
 * conf value http_gzip, if set, is used in xapp internal output buffer handling to compress als
 * HTTP response output with gzip. if left to null|false nothing will be compressed
 *
...
/**
 * the date format used within the application as defined on php.net/manual/en/function.strftime.php
 * acts as a default value through out the application
 *
...
/**
 * the datetime format used within the application as defined on php.net/manual/en/function.strftime.php
 * acts as a default value through out the application
 *
 * @const XAPP_CONF_DATETIME_FORMAT
 */
...
/**
 * the xapp console driver if used will route all calls from xapp_console to driver instance for inline
 * browser console logging. can only be used when xapp base class is used. use value null or false if you dont
 * want to use console at all. supported drivers are "firephp" and "chromephp".
 *
...
/**
 * xapp is capable of java style import through dot and wildcard notation e.g. xapp_import('foo.Base') or
 * xapp_import('foo.Base.*') to load packages. the default import path is the xapp base path. you can
 * define usr import paths by passing an string or array of absolute paths (!) to this conf constant
 * when settings xapp config values in your app. NOTE that import dirs must be readable and exists no
 * errors are thrown!
 *
...
// *****************************************************************
// init xapp core by auto setting paths and default conf values to
// be stored in global scope for latter use. require core files
// *****************************************************************
...
// *****************************************************************
// include xapp base files if xapp is used with xapp base functionality
// required if xapp is used as app builder rather then single modules
// *****************************************************************
...
/**
 * global conf setter/getter function for xapp that will used inside xapp base class if used or
 * elsewhere with custom implementation. to initialize the first time the function must be called
 * either with an array of conf option to override defaults or call with out parameter to initialize
 * default values. after init the function acts as getter/setter getting an option if first parameter
 * is only present. set values if both parameter are set.
 * NOTE: if you want to make use of xapp as application base user xapp base class and pass conf array
 * to xapp::run.
 *
...
/**
 * global path getter/setter function to set all system wide path and store any other path into the
 * global path array. to auto initialize all default xapp paths the function should be called without
 * any parameters. use the function then as setter/getter to get xapp path and set new paths if required.
 * if no parameter is passed will return the whole path array
 * NOTE: be careful and dont overwrite xapp default paths!
 *
...
/**
 * setter/getter function to set/get system wide environment vars be using phps build
 * in env function putenv/getenv. returns boolean when setting to test if setting
 * was successful and the env kex return value when getting.
 *
...
/**
 * setter/getter function to set/get system wide php ini values returning the ini value,
 * false if unable to set or an array with all ini values
 *
...
/**
 * function to check if xapp base class is used/loaded or if any other class
 * is a xapp class belonging to the xapp framework.
 *
 * @param   null|object|string $class expects instance of a any object, string name of class
 *          or null to just check if xapp base class is loaded
...
/**
 * shortcut function to validate values if of expected type looking for phps native functions and xapps
 * internal data type validation functions
 *
...
/**
 * xapp java style dot and wildcard class/package loader imports/requires class or
 * packages from the default xapp base path or paths passed as conf value defined
 * by XAPP_CONF_IMPORT_PATH. the import $parameter can be:
 *
 ...
 *
 * the function store all imported java style classes and packages into a global
 * package cache to check if package has been already imported and therefore does
 * not execute any import code. the function does not throw any error but simply
 * tries to require the $import param with triggering php error when $import value
 * require fails.
 *
...
function xapp_import($import)
{
    ...
    if(preg_match('/(.*)\.(php|php5|phps|phtml|inc)$/i', $import, $m))
    {
        ...
        if(substr($import, -1) !== '*')
        {
            ...
            if(func_num_args() >= 2 && strpos(func_get_arg(1), DS) !== false)
            {
                $path = (array)func_get_arg(1);
            }else{
                $path = xapp_path(XAPP_PATH_BASE);
                $path = array_diff(array_merge((array)$path, (array)xapp_conf(XAPP_CONF_IMPORT_PATH)), array(''), array(1));
            }
                ...
                if(($dir = @opendir($p)) !== false)
                {
                    while(($file = readdir($dir)) !== false)
                    {
                        if($file === '.' || $file === '..' || stripos($file, '.svn') !== false)
                        {
                            ...
                            xapp_import(trim($import, '.*') . '.' . $file . '.*', $b);
...
// *****************************************************************
// xapp overwritable/hookable functions for system wide functionality.
// each of the following function can be overwritten by defining the
// function before the xapp core.php file is included. see function
// description on how to overwrite
// *****************************************************************

if(!function_exists('xapp_event'))
{
    /**
     * xapp event trigger/listen shortcut function to (re)delegate re(direct)
     * xapp events. the events are triggered from inside the xapp framework and can
     * be listened to from everywhere if Xapp_Event class is loaded. the event
     * syntax is a following (action):(namespace).(event) to trigger events:
     * "trigger:xapp.shutdown", "trigger:myevent", to listen to events:
     * "listen:xapp.shutdown", "listen:xapp.myevent". the xapp event functionality
     * can be used outside of xapp framework and new events easily defined and triggered.
     * best to use own events with own namespace like "trigger:my.event". if this function
     * is overwritten events can be handled with custom logic and redirected.
     *
     * overwrite like:
     * <code>
     ...
     * @param   null|callable|array $mixed expects according to event action a callable for
     *          listeners or a params array to be passed to the listener from the trigger
     * @param   boolean $bool expects a boolean value which is used in xapp listener mode
     *          to reset listeners for event. in trigger mode to stop bubbling events.
...
{
    /**
     * xapp profiler shortcut function is used system wide to redirect profile data/action back to concrete
     * implementation which defaults to Xapp_Profile class if xapp is used. this function can be overwritten to
     * implement custom profiling logic. calling this function with all parameters null or only first parameter not null
     * should be used to emulate a tick function. if the second parameter is a callable can emulate a tick function with callback or
     * a time the execution of the call back. the third optional parameter can be used to pass a microtime value representing a start
     * value or a already timed value. the first parameter could also be a boolean false value expecting the profiler to stop working
     *
     * overwrite like:
     * <code>
...
{
    /**
     * xapp debug shortcut function used system wide in xapp framework and can be overwritten.
     * if xapp base class is used will redirect debug messages back to xapp::d debug function unless
     * xapp conf constant XAPP_CONF_DEBUG_MODE is set to 4 or "console" redirecting all debug messages directly
     * to php browser console via the Xapp_Console wrapper class.
     *
     * overwrite like:
     * <code>
     ...
     * </code>
     *
     * @param string $m expects the debug message
...
{
    /**
     * xapp error shortcut function used system wide to redirect errors back to xapp base class
     * if xapp base class is used. this function can be easily overwritten to handle errors thrown
     * by exceptions and internal php errors channeled through xapps internal error handler defined
     * in XAPP_CONF_HANDLE_ERROR. use the the this function in all derived exception classes inheriting
     * from phps native ErrorException class in constructor - see Xapp_Error for example. when
     * implementing your own error handler you can bounce back your errors to this function or overwrite
     * it. if xapp base class is not used with direct error to default error handling defined either via
     * phps set_error_handler function or error_log function and triggered by phps trigger_error function.
     * this functions expects exception as well as errors passed as it there where an exception but only
     * for logging purpose or even act as set_error_handler if needed. when redirecting all errors to
     * console you must load and enable Xapp_Console by defining an external console driver like "firephp"
     * and define XAPP_CONF_HANDLE_ERROR with value "console".
     *
     * overwrite like:
     * <code>
     ...
     * </code>
     *
     * @param   string|Exception|Xapp_Error|array $e expects either string error message, instance of Exception or
     *          array return from phps native function error_get_last
...
// *****************************************************************
// xapp option handling is a unique feature that can be used in any xapp
// build in or custom class. xapp option handling will ensure that a)
// always the required options are set, b) only the options are
// passed which exists, c) the options value data type is valid as
// defined in the options dictionary. to use this feature see a reference
// class e.g. Xapp_Log_Error on how it works. to initialize instance
// options use either xapp_set_options, or xapp_init_options to also validate
// if required options are set. the xapp options handling can be used out
// of the box to build generic options arrays with setter/getter to use
// for array pooling. the option handling will work with any class also that
// as a minimum implements public static/non-static property $options.
//
...
{
    /**
     * xapp option shortcut xapp_init_options
     * initializes the passed options in first parameter according to second parameter which can
...
{
    /**
     * xapp option shortcut xapp_set_options
     * sets options from first parameter to second parameter merging with existing options where
...
{
    /**
     * xapp option shortcut xapp_set_option
     * single option setter sets option from first parameter to third parameter as part of
     * the xapp option functionality. the third parameter can be:
     * 1) array = setting $key to $mixed
     * 2) object = object instance implementing xapp option handling or not
     * 3) string = object class name as string to handle static classes/options
     * 4) null = null when called inside class for xapp option handling for auto caller determination
     *
     * use the third parameter to reset the array before setting option. this will only have affect
     * on class options managed by xapp option handling
     *
...
{
    /**
     * xapp option shortcut xapp_get_options
     * full option getter gets all options as part of
     * the xapp option functionality. the first parameter can be:
...
{
    /**
     * xapp option shortcut xapp_get_option
     * single option getter to get option $key, the first parameter from $mixed the
     * second parameter returning third parameter $default if not found. the second
     * parameter can be:
...
{
    /**
     * xapp option shortcut xapp_has_option
     * check whether first parameter the option $key exists in second parameter which can be:
     * 1) array = array to check for key
     * 2) object = object instance implementing xapp option handling or not
     * 3) string = object class name as string to handle static classes/options
     * 4) null = null when called inside class for xapp option handling for auto caller determination
     * if the third parameter is true will not only set for existence but also for a value
     * which is not null and not false and is not empty
     *
...
{
    /**
     * xapp option shortcut function for xapp_has_option
     * this will use xapp_has_option with third parameter strict = true so option value has to be
     * a valid value other then null, false, empty array. see xapp_has_option
     *
     * @see xapp_has_option
     * @param null|string $key expects the options key name to check
     * @param null|object|string|array $mixed expects value according to explanation above
...
{
    /**
     * xapp option shortcut xapp_unset_option
     * will unset first parameter option $key from second parameter which can be:
...
{
    /**
     * xapp option shortcut xapp_reset_option
     * reset an option value making sure that in case when xapp option handling is used arrays
     * are fully resetet and not merged when setting options
     * the xapp option functionality. the third parameter can be:
     * 1) array = setting $key to $mixed
     * 2) object = object instance implementing xapp option handling or not
     * 3) string = object class name as string to handle static classes/options
...
{
    /**
     * xapp option shortcut xapp_reset_options
     * will reset first params options which can be:
     * 1) array = reset array
     * 2) object = object instance implementing xapp option handling or not
     * 3) string = object class name as string to handle static classes/options
     * 4) null = null when called inside class for xapp option handling for auto caller determination
