...
class Xapp_Rpc_Fault extends ErrorException
{
    ...
    public static function t($message, $code, $severity = XAPP_ERROR_IGNORE, Array $data = null)
    {
        ...
        $self = new self($message, (($fault !== null) ? $fault : $code), $severity);
    ...
    /**
     * set additional data in key => value pairs. the first parameter must not be set
     * if wanting to add none named parameters to data array
     *
     * @error 14307
    ...
    /**
     * get data from additional data array either by passing first parameter key returning the data
     * for that key or without first parameter returning the complete data array. if key is not found
     * in data array will return default second parameter
     *
     * @error 14308
    ...
    /**
     * check if data exists in additional data array. if first parameter is null will check if anything
     * has been set yet or if data array is empty. if first parameter is set will check if key exist in
     * additional data array. use second parameter to check for strict value - a value that is not null,
     * not empty and not false.
     *
     * @error 14309
     * @param null|int|string $key expects the optional key to get data for
    ...
    public function hasData($key = null, $strict = false)
    {
        if($key !== null)
        {
            if((bool)$strict)
            {
                return (array_key_exists($key, $this->_data) && xapp_is_value($this->_data[$key])) ? true : false;
            }else{
                return (array_key_exists($key, $this->_data)) ? true : false;
            }
        }else{
