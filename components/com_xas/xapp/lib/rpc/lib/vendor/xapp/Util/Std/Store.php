...
class Xapp_Util_Std_Store /*implements Serializable*/
{
            ...
            if(is_object($mixed))
            {
                ...
                if(is_file($mixed))
                {
                    ...
                    if(($mixed = file_get_contents($mixed)) !== false)
                    {
                        ...
                        throw new Xapp_Util_Std_Exception("unable to read from file: $mixed", 1700101);
                    }
                }else{
                    if(is_string($mixed) && preg_match('/^([adObis]\:|N\;)/', trim($mixed)))
                    {
                        ...
                        throw new Xapp_Util_Std_Exception("passed first argument in constructor is not a valid object or file path", 1700102);
                    }
                }
            }
        }
    ...
    /**
     * init function resets values used once a query result has been obtained to reset connector chain
     *
     * @error 17003
     * @return $this
     */
    ...
    public function merge($mixed1, $mixed2)
    {
        ...
        if(xapp_type($_mixed1) === xapp_type($mixed2))
        {
            ...
            throw new Xapp_Util_Std_Exception("merging values at paths only allowed for values of same type", 1701701);
        }
    }
    ...
    public function serialize($path = null)
    {
        if($path !== null)
        {
            if(is_string($path))
            {
                ...
                throw new Xapp_Util_Std_Exception("first parameter: \$path must be a string", 1701801);
            }
        }else{
    ...
    public function &unserialize($path)
    {
        if(is_string($path))
        {
            ...
            throw new Xapp_Util_Std_Exception("first parameter: \$path must be a string", 1701901);
        }
    }
     ...
     * $store->get($objet, array('/root', array('id=1');
     *
    ...
    public static function _set(&$object, $mixed = null, $value = null, $extend = true, $position = null, $typesafe = false)
    {
        ...
        if($result !== false)
        {
                    ...
                    if($position !== null)
                    {
                        ...
                        if($typesafe)
                        {
                            if(xapp_type($result) === xapp_type($value))
                            {
                                ...
                                throw new Xapp_Util_Std_Exception("value found at path can not be overwritten due to typesafe protection", 1702101);
                            }
                        }else{
                            $result = $value;
                        }
            ...
            if($extend && is_object($parent))
            {
                ...
                throw new Xapp_Result_Exception("no result found for query", 1702102);
            }
        }
    }
    ...
    public function save()
    {
        if($this->_file !== null)
        {
            if(file_put_contents($this->_file, self::encode($this->_object), LOCK_EX))
            {
                ...
                throw new Xapp_Util_Std_Exception("unable to save to file: " . $this->_file, 1702501);
            }
        }else{
    ...
    public function saveTo($file)
    {
        if(file_put_contents($file, self::encode($this->_object), LOCK_EX))
        {
            ...
            throw new Xapp_Util_Std_Exception("unable to save to file: $file", 1702601);
        }
    }
