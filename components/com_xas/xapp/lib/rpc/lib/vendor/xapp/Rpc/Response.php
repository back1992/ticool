...
abstract class Xapp_Rpc_Response
{
    ...
    /**
     * contains all data that need to converted to json to be flushed out as string.
     * data should always have an key => value pair
     *
     * @var array
     */
    ...
    public static function factory($driver, $options = null)
    {
        ...
        if(class_exists($class, true))
        {
            ...
            throw new Xapp_Rpc_Response_Exception("rpc response class: $class does not exist", 1450201);
        }
    }
        ...
        if($mixed !== null)
        {
            ...
            if(is_string($mixed) && $value === null){
                ...
                if(sizeof($mixed) === 2)
                {
                    ...
                    throw new Xapp_Rpc_Response_Exception("response raw header passed is not valid", 1450501);
                }
            //header with key => value pair in first and second parameter
            }else if(is_string($mixed) && is_string($value)){
                $mixed = array(array(trim($mixed), trim($value)));
            //header not recognized
            }else{
                throw new Xapp_Rpc_Response_Exception("response header passed is not recognized", 1450502);
            }
            for($i = 0; $i < sizeof($mixed); $i++)
            {
                if(isset($mixed[$i][0]) && isset($mixed[$i][1]))
                {
                    ...
                    throw new Xapp_Rpc_Response_Exception("response header miss match when passing keys and values as arrays", 1450503);
                }
            }
        }else{
    ...
    protected function flushHeader()
    {
        if(!$this->_headerSent)
        {
            ...
            {
                header($h->name, $h->value, $h->replace, $h->code);
            }
    ...
    public function __get($name)
    {
        if($this->has($name))
        {
            ...
            throw new Xapp_Rpc_Response_Exception("overloading property only allowed for data parameters", 1451301);
        }
    }
    ...
    public function __unset($name)
    {
        if($this->has($name))
        {
            ...
            throw new Xapp_Rpc_Response_Exception("overloading and unsetting property only allowed for data parameters", 1451501);
        }
    }
    ...
    public function __call($name, Array $arguments = array())
    {
        if(empty($arguments) && $this->has($name))
        {
            ...
            throw new Xapp_Rpc_Response_Exception("overloading method only allowed to get values from data array", 1451601);
        }
    }
