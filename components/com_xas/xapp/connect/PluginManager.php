 ...
 * @author mc007
 */
class XApp_PluginManager implements Xapp_Singleton_Interface {
    ...
    public function loadPluginWithConfiguration($pluginPrefix,$pluginConfig){
        ...
        //xapp_dumpObject($pluginConfig,'cfg');
        //xapp_dumpObject($pluginPrefix,'prefix');
        ...
        if(!is_array($pluginConfig)){
            ...
            //determine load path
            $pluginPath = $pluginPrefix . DS . $pluginConfig->location . DS . $pluginConfig->name . '.php';
        }else{
     ...
     *
     */
    public function loadPlugins($searchPath,$pluginPrefix='',$type=''){

        //error_log('loading plugins from : ' . $searchPath . ' for type : ' . $type);
    ...
    public function getPluginInfos($searchPath,$type){
        ...
        //scan for plugin configs
        $foundPlugins = $this->getEnabledPluginsConfigs($searchPath);
        //error_log('getting config : ' . $searchPath . ' : ' . $type);
    ...
    public function getEnabledPluginsConfigs($searchPath){
        ...
        foreach($root as $value)
        {
            ...
            if(is_file($_pathToSearch . DS . $value) && strpos($value,'json')!==false)
            {
                $confStore=$this->readJSON($_pathToSearch . DS . $value,'json');
                if($confStore){
    ...
    public function createFakePluginInstance($name,
        ...
        $pluginOptions=array(
            ...
            //cache configuration
            Xapp_Connect_Plugin::CACHE_CONF=>array(
            ...
            Xapp_Connect_Plugin::LOGGING_CONF=>$plgLoggingConf,
            //service configuration
            Xapp_Connect_Plugin::SERVICE_CONF=>$plgServiceConf
        );
